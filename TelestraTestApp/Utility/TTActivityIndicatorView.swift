...
public class TTActivityIndicatorView: NSObject {
    ...
    fileprivate static let shared: TTActivityIndicatorView = {
        let instance = TTActivityIndicatorView()
    ...
    private override init() {
        ...
        if let windowObj = delegate.window {
            ...
            window = UIApplication.shared.keyWindow!//windows.filter {$0.isKeyWindow}.first
        }
        statusLabel = nil
        backgroundView = nil
    ...
    fileprivate func configureActivityIndicator(withStatusMessage message: String, isUserInteractionEnabled userInteractionStatus: Bool) {
        ...
        if spinnerContainerView != nil {
            ...
            TTActivityIndicatorStyle.createSpinner(in: viewLayer, size: animationRectsize, color: spinnerColor, style: activityIndicatorStyle)
        }
